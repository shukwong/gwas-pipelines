...
task combine_bolt_results {
    ...
    command <<<
        ...
        echo -e "CHR\tPOS\tSNP\tTested_Allele\tOther_Allele\tBETA\tSE\tP\tN" > bolt_~{pheno_col}_~{setname}_~{batch_name}_results_merged.tsv

        cat ~{sep=' ' imputed_stats_files} | gzip -d | grep -v ^SNP | awk -v N_var=$N '{print $2"\t"$3"\t"$1"\t"$5"\t"$6"\t"$11"\t"$12"\t"$14"\t"N_var}' >> bolt_~{pheno_col}_~{setname}_~{batch_name}_results_merged.tsv
        
        gzip bolt_~{pheno_col}_~{setname}_~{batch_name}_results_merged.tsv
    >>>
